...
class Group_LDAP extends BackendUtility implements \OCP\GroupInterface, IGroupLDAP, IGetDisplayNameBackend {
	...
	private function _groupMembers($dnGroup, &$seen = null) {
		...
		if ($this->access->connection->ldapNestedGroups
			...
			&& $this->access->connection->ldapMatchingRuleInChainState !== 'unavailable'
		) {
			...
			if (!empty($memberRecords)) {
				if ($this->access->connection->ldapMatchingRuleInChainState === 'unknown') {
					$this->access->connection->ldapMatchingRuleInChainState = 'available';
